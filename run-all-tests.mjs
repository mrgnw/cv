...
async function runAllTests() {
  ...
  const tests = [
    {
      ...
      description: 'Validates that experience data files are consistent and properly structured'
    },
    {
      name: 'Version Reader Integration',
      script: 'test-version-integration.mjs',
      description: 'Tests end-to-end version merging and route-level data flow'
    },
    {
      name: 'Experience Data Validation',
      script: 'validate-experience.mjs',
      description: 'Comprehensive validation of experience data integrity and consistency'
    }
  ];
  ...
  if (allPassed) {
    ...
    log('✅ National Care Dental end date (2025-03-17) is correct', 'green');
    log('✅ Version customizations work without breaking base data', 'green');

    log('\n💡 CONCLUSION:', 'bold');
    log('Your concern about experience data not making it to the main page', 'green');
    log('appears to be unfounded. The system is working as expected!', 'green');
  } else {
    log('❌ SOME TESTS FAILED', 'bold');
    log('Please review the failed tests above and address the issues.', 'red');

    const failedTests = testResults.filter(r => !r.passed).map(r => r.name);
  ...
  if (allPassed) {
    ...
    log('3. 📄 Run PDF generation to ensure all versions export correctly', 'blue');
